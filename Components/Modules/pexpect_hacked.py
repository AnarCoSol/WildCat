'''Pexpect Modules hacked on June 2016 by anar-co_sol, for original pexpect
introduction/helpinhg lines import the original modules, then help(pexpect).

list of hacked lines:
- (at lb: 264) __init__, method of spawn class, added two attributes necessary for the next method hacked
- (at ln: 1581) __interact_copy , method of spawn classs, in order to implement
the TMN (Trinity-Matrix-Neo) interface.

Simple example to implement tne Matrix role in TMN by using this module:

________________Matrix.py_______________________
import pxpt_hacked as pexpect
...
class spawn(object):
    ...
    def __init__(self, command = str(), args=[], timeout=30, maxread=2000,
        searchwindowsize=None, logfile=None, cwd=None, env=None,
        ignore_sighup=True, in_file = str(), out_file = str(), mode = str(), echo = True ): #hacked attributes added: in_file, out_file
    ...
    def __interact_copy(self, escape_character=None,#hacked, it seems it works..
            input_filter=None, output_filter=None):
        ...
        tm = Text_manipulator()#
        tm.silent = True#
        #echo = False available only for pair f_in - f_out (as 'io file', or partially as 'io cat'
        def f_in(s):
            while True:
                data = tm.Read_as_input(s.in_file)# self.custom_in(argvs)
                ...
                if i != -1:
                    data = data[:i]
                    s.__interact_writen(s.child_fd, data)
                    break#Warning!!
                
                if not s.echo:
                    s.no_echos.append(data)
                
                s.__interact_writen(s.child_fd, data)
            
        def f_out(s):
            #child_fd = file_descriptor <- is an int (ie a pid)
            while True:
                data = str()
                echo_data = str() #maybe not needed
                r, w, e = s.__select([s.child_fd, s.STDIN_FILENO], [], [])
                if s.child_fd in r:
                    try:
                        data = s.__interact_read(s.child_fd)
                    except OSError as e:
                    ...
                    if not s.echo:
                        if s.no_echos != list():
                            ...
                            if data.count(s.no_echos[-1]):
                                data = data[:data.index(s.no_echos[-1])] + data[data.index(s.no_echos[-1]) + len(s.no_echos[-1]):]
                    ...
                    if output_filter:
                        ...
                        if echo_data != str():
                            echo_data = output_filter(echo_data)

                    #if not data: continue
                    if not echo_data:
                        continue
                    
                    if s.logfile is not None:
                        s.logfile.write(data)
                        s.logfile.flush()
                    
                    os.write(s.STDOUT_FILENO, echo_data)
                    
                    tm.Write_as_output(s.out_file, data)# self.custom_out(argvs)
                    
        def first_file(s): #is not implemented jet echo False in first_file
            data = tm.Read(s.in_file).split("\n")[:-1] #prevent unexpected \n
            
            for d in data:
                
                time.sleep(0.50) ##sketchy adjustment
                ...
                i = d.rfind(escape_character)
                if i != -1:
                    d = d[:i]
                    s.__interact_writen(s.child_fd, d)
                    break#Warning!!
        ...
        def inverted_in(s):
            while True:
                ...
                #os.write(self.STDOUT_FILENO, data)
                tm.Write_as_output(s.in_file, data)
                

        def inverted_out(s):
            while True:
                try:
                    data = tm.Read_as_input(s.out_file)
                except OSError as e:
                    # The subprocess may have closed before we get to reading it
                    if e.errno != errno.EIO:
                        raise
        ...
        if self.mode == "i direct" or self.mode == "o file":
            func_in = direct_in
            func_out = f_out
            
        elif self.mode == "o direct" or self.mode == "i file":
            func_in = f_in
            func_out = direct_out
            
        elif self.mode == "io direct":
            func_in = direct_in
            func_out = direct_out
            
        elif self.mode == "io file":
            func_in = f_in
            func_out = f_out

        elif self.mode == "io inverted":
            func_in = inverted_in
            func_out = inverted_out
        ...
        if self.mode == "io cat" :# need better arg than "io cat"
            func_in = first_func #rigid: file first then i file, more args needed
            func_out = f_out

        t_out = Thread_launcher(func_out, self)
        t_in = Thread_launcher(func_in, self)
